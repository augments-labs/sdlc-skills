...
- **Run the gates before you commit:** `bash scripts/sh/validate-skills.sh`, `bash scripts/sh/validate-skill-graph.sh`, and `bash scripts/sh/token-budget.sh --chain NAME` for each chain in `docs/chains.toml`. CI runs them on every push and PR.
