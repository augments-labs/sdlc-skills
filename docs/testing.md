...
| `tests/`, offline | The checked packaging or script predicate | `run-session-start.sh`, `run-plugin-smoke.sh`, `run-sdd-scripts.sh`, `run-opencode-plugin.sh`, `run-pi-extension.sh`, `run-serve-preview.sh`, `run-validate-skills.sh` |
...
- **Adapters and hooks:** run `tests/run-session-start.sh` and
  ...
  `subagent-driven-development` script changes; and `tests/run-validate-skills.sh`
  when `scripts/sh/validate-skills.sh` changes.
