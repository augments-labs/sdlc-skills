...
| pi | `.pi/extensions/sdlc-skills.js`, registered through the `pi` key in `package.json` | `before_agent_start` router injection via `appendSystemPrompt`; `session_compact` re-appends it into the saved compaction summary. The offline test proves the extension's own registration and injection; the harness test proves the package is installed and loaded, since 0.86.1 has no non-interactive skill dump |
