...
2. Bump the version in all six manifests: `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `.kimi-plugin/plugin.json`, `.muse-plugin/plugin.json`, `plugins/sdlc-skills/.codex-plugin/plugin.json`, and the root `package.json` (the npm-installable OpenCode plugin package). The gate fails if they disagree, so a half-done bump cannot ship.
